...
n_MSA_realEstate<- n_MSA_realEstate %>% 
  ...
  mutate(weighted_avg_rent_change = 0.10 * n_avg_rent_change) %>% 
  mutate(weighted_med_rent_change = 10* n_med_rent_change) %>% 
  ...
  mutate(med_total_score = weighted_pop_change + weight_emp + weighted_occ + weighted_med_rent_change + weighted_med_rent_income + weight_income_homeprice) %>% 
  mutate(avg_total_score = weighted_pop_change + weight_emp + weighted_occ + weighted_avg_rent_change + weighted_avg_rent_income + weight_income_homeprice)
```
...
n_MSA_realEstate <- n_MSA_realEstate %>% 
  ...
  mutate(real_estate_score_med = msa) %>% #this is just a place holder
...
for (i in 1:size[1]) {
  ...
  n_MSA_realEstate$real_estate_score_med[i] = weighted.mean(MSA_x_data, real_estate_weights)
}
