...
MSA_realEstate_compiled<- MSA_median_rent_data %>% 
  select(cbsa, rent50_0) %>%
  mutate(median_rent = as.numeric(rent50_0)) %>% 
...
MSA_RE_ratios <- MSA_realEstate_compiled %>% 
  ...
  mutate(avg_rent_to_income = (avg_rent_2021*12)/median_income_usd_2021) %>% 
  mutate(med_rent_to_income = (median_rent*12)/median_income_usd_2021)
...
n_MSA_realEstate <- MSA_RE_ratios %>% 
  ...
  mutate(n_rent_change = (avg_rent_change - min(avg_rent_change, na.rm = TRUE)) / (max(avg_rent_change, na.rm = TRUE) - min(avg_rent_change, na.rm = TRUE))) 
...
```{r}
#FILTER BY NEW MSA LIST
new_MSA_list <- unique(select_MSAs$cbsa)

n_MSA_realEstate_select_MSAs <- n_MSA_realEstate %>% 
  filter(cbsa %in% c(new_MSA_list))
```
