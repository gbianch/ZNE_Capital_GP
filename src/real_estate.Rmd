...
MSA_realEstate_data <- MSA_realEstate_data %>% 
  mutate(income_to_rent_inverted = 1 - n_income_to_rent)

```

```{r}
# weights from owen
real_estate_weights <- c()
...
for (i in 1:size[1]) {
  MSA_x_data <- c(MSA_realEstate_data$eviction_notice_days_normalized_inverted[i],
                  MSA_realEstate_data$security_depopsit_limit_num[i],
                  MSA_realEstate_data$notice_for_entry_days_num[i]
                  )
