...
MSA_realEstate_data <- city_realEstate_data %>% 
  ...
  mutate(rent_change21_22 = ((avg_rent_2022- avg_rent_2021)/avg_rent_2021)*100)
...
```{r}
# weights from owen
real_estate_weights <- c(0.35, 0.20, 0.15, 0.10, 0.10, 0.10, 0.10)
...
weighted_RE_data <- n_MSA_realEstate %>% 
  mutate(weighted_pop_change = 0.3 * n_pop_change) %>% 
  mutate(weight_emp = 0.25 * n_employ_change) 
