...
MSA_landlord_data <- MSA_landlord_data %>% 
  mutate(
    security_depopsit_limit_num = case_when(
      ...
      security_deposit_limit == "1_month_rent" ~ 0.5,
      security_deposit_limit == "2_months_rent" ~ 0.0))
...
landlord_weights <- c(0.25, 0.25, 0.25, 0.25)
...
for (i in 1:size[1]) {
  MSA_x_data <- c(MSA_landlord_data$rent_control_binary[i], 
                  MSA_landlord_data$security_depopsit_limit_binary[i],
                  MSA_landlord_data$notice_for_entry_days_binary[i],
                  MSA_landlord_data$eviction_notice_days_normalized_inverted[i])
  MSA_landlord_data$landlord_score[i] = weighted.mean(MSA_x_data, landlord_weights)
}
```

```{r}
#test Virginia new laptop on github
#testing github token setupf
```
