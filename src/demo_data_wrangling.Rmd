...
msa_pop_2021_data <- read_csv(here("data","raw", "msa_2021_alldata.csv")) %>% 
  ...
  separate(col = "name",
           ...
           sep = ", ") %>% 
  mutate(pop_percent_change = (popestimate2021-popestimate2020)/popestimate2021 * 100)  # add population % change column 
...
potential_MSAs <- msa_pop_2021_data %>% 
  ...
  select(cbsa, MSA, state, pop_percent_change20_21, normalized_pop2021) 
...
```{r fig.height = 7, fig.align='center', eval = FALSE}
...
MSA_income_data <- read_csv(here("data","raw", "MSA_income", "2021_data", "2021_income_data.csv")) %>% 
  row_to_names(1) %>% 
  clean_names() %>% 
...
income_potential_MSAs <- MSA_income_data %>% # rename row names
  ...
  select(cbsa, MSA, state, median_income, pop_percent_change20_21, normalized_pop2021) #reorder columns
#select(cbsa, MSA, state, pop_percent_change20_21, median_income, income_moe_USD) 
...
MSA_employment_yoy <- MSA_employment %>% 
  filter(year >= 2020) %>% 
  filter(month == "01") %>% 
  arrange(MSA, month) %>% 
  group_by(MSA) %>% 
  mutate(YoY = (employment_rate-lag(employment_rate))/lag(employment_rate)*100) %>% 
  mutate(employment_YoY = round(YoY, digits = 3)) %>% 
  mutate(emp_pos = employment_YoY >= 0) %>%# function to create column for yoy value, true = positive YoY
  inner_join(avg_emp_yoy) %>% 
  filter(year == "2022") %>% 
  select(!c(YoY, month, year)) 
...
portland_employment_data <- read_csv(here("data", "raw", "portland_me_employment.csv")) %>% 
  row_to_names(row_number = 10) %>% 
  clean_names() %>% 
  select(year, jan) %>% 
  rename(civilian_labor = jan)
...
```{r eval = FALSE}
# average employment from 2000 to 2022
employment_summary_since2000 <- MSA_employment %>% 
  group_by(MSA) %>% 
  filter(year >= 2000) %>% 
  summarize(
    avg_employment = mean(employment_rate),
    sd_employment = sd(employment_rate)
  )

```

```{r employment visuals, fig.align = "center", fig.show="hold", fig.height= 7, fig.width=10}
...
```{r payroll data, eval = FALSE}
# payroll data has not been added to final table
payroll_data <- read_csv(here("data","raw", "employee_payroll_MSA21_22.csv")) %>% 
  row_to_names(4) %>% 
  clean_names() %>% 
  rename(MSA = na) %>% 
  rename(payroll_change_may21 = may_2021) %>% 
  rename(payroll_change_mar22 = mar_2022) %>% 
  rename(payroll_change_apr22 = apr_2022) %>% 
  rename(payroll_change_may22 = may_2022_p)

MSA_payroll <- payroll_data %>% 
  full_join(potential_MSAs) %>% 
  drop_na() %>% 
  pivot_longer(cols = 2:5,
               names_prefix = "payroll_change_",
               names_to = "year",
               values_to = "payroll_change") %>% 
  mutate(payroll_change = as.numeric(payroll_change)) %>% 
  group_by(MSA, year)  %>% 
  mutate(payroll_pos = payroll_change >= 0)
   
```
```{r payroll visual, fig.height = 7, eval = FALSE}
ggplot(MSA_payroll, aes(x = payroll_change, y = reorder(MSA, payroll_change), fill = payroll_pos)) +
  geom_col(position = "identity", color = "black", size = 0.25) +
  scale_fill_manual(values = c("red", "darkolivegreen3"), guide = FALSE) +
  labs(x = "percent change",
       y = "",
       title = "Quarterly change in employee payroll from 2021 to 2022") +
  facet_grid(~year)
...
MSA_median_rent2021_data <- read_csv(here("data", "raw", "2021MSA_medianrent.csv")) %>% 
  ...
  mutate(median_rent2021 = as.numeric(rent50_0)) %>% 
...
MSA_median_rent2022_data <- read_csv(here("data", "raw", "2022MSA_median_rent.csv")) %>% 
  ...
  separate(col = cbsa,
           ...
           sep = -5) %>% 
  mutate(cbsa = as.double(cbsa)) %>% 
  mutate(median_rent2022 = as.numeric(rent_50_0)) %>% 
  select(cbsa, median_rent2022)

potential_MSAs <- potential_MSAs %>% 
  right_join(MSA_median_rent2022_data) %>% 
  drop_na()
...
```{r house price data}
metro_house_cost_data <- read_csv(here("data","raw", "metro_home_costs", "metro_home_prices.csv")) %>% 
  row_to_names(3) %>% 
  clean_names() %>% 
...
metro_house_prices <- MSA_house_income %>%
  left_join(metro_house_cost_data)  %>% 
  rename(house_price_2020 = x2020) %>% 
  rename(house_price_2021 = x2021_r) %>% 
  rename(houseprice_change21_22 = q1_q1) %>% 
  select(!c(metro_area, x2019, na)) %>% 
  # select(cbsa, houseprice_change21_22) %>% 
  separate(col = "houseprice_change21_22",
           into = "houseprice_change21_22",
           sep = "%") %>% 
  mutate(houseprice_change21_22 = as.numeric(houseprice_change21_22))
```

```{r house price exploratory, eval = FALSE}
ggplot(data = metro_house_cost_data, aes(x = log(x2020))) +
   geom_histogram(bins = 100)

ggplot(data = metro_house_cost_data, aes(sample = log(x2020))) +
  geom_qq() 
...
Source:[U.S. Census Bureau](https://data.census.gov/cedsci/table?q=property%20tax&g=0100000US%2431000M1&d=ACS%201-Year%20Estimates%20Detailed%20Tables&tid=ACSDT1Y2019.B25103&tp=true)

```{r property taxes data wrangling, eval = FALSE}
property_tax_data <- read_csv(here("data","raw", "wide_property_tax_data.csv")) 

# create new data frame with tax estimate values
median_tax_df <- property_tax_data %>% 
  select(ends_with("estimate")) %>% 
  slice(2) %>% 
  stack() %>% # flip data table to be tidy
  rename(MSA = ind) %>% 
  mutate(median_tax_USD = as.numeric(values)) %>% 
  separate(col = MSA,
           into = c("MSA", "state"),
           sep = ", ") %>% 
  separate(col = state,
...
# make data frame with margin of error values
moe_tax_df <- property_tax_data %>% 
  select(ends_with("error")) %>% 
  slice(2) %>% 
  stack() %>% # flip data table to be tidy
  rename(MSA = ind) %>% 
  separate(col = values,
           into = c("value", "tax_moe_USD"),
           sep = 1) %>% 
  mutate(tax_moe_USD = as.numeric(tax_moe_USD)) %>% 
  separate(col = MSA,
           into = c("MSA", "state"),
           sep = ", ") %>% 
  separate(col = state,
...
MSA_property_tax <- metro_house_prices %>% 
  full_join(median_tax_df) %>% 
  left_join(moe_tax_df) %>% 
  drop_na(cbsa) # drop all MSA rows if NA in cbsa column
  
```

```{r property tax visual, fig.height = 7, fig.align='center', eval = FALSE}
ggplot(MSA_property_tax, aes(x = median_tax_USD, y = reorder(MSA, median_tax_USD))) +
  geom_errorbarh(aes(xmin = median_tax_USD - tax_moe_USD, xmax = median_tax_USD + tax_moe_USD)) + 
  geom_point(size = 3, color = "darkgreen") + 
  theme_minimal(base_size = 12.5) + 
  labs(title = "Median property tax in 2019", 
       subtitle = "", 
       x = "Median Property Tax estimate (in USD)", 
       y = "") + 
  scale_x_continuous(labels = label_dollar())
```
...
```{r join population, unemployment, income, and house price data}
# join employment data with income, house prices, tax, and population data
MSApop_income_housecost_tax_emp <- MSA_employment_yoy %>%
  filter(year == "2022") %>% 
  mutate(employment_rate = round(employment_rate, digits = 3)) %>% 
  select(cbsa, year, employment_rate, employment_YoY) %>% 
  # right_join(MSA_property_tax) %>% 
  select(!year)
...
datatable(MSApop_income_housecost_tax_emp, extensions = 'Buttons', options = list(
    dom = 'Bfrtip',
...
msa_seperated <- MSApop_income_housecost_tax_emp %>% 
  separate_rows(MSA, sep = "-") %>% 
  rename(city = MSA)# %>% # seperate MSA cities into their own columns
 # full_join(payroll_data, by = c("MSA" = "area")) %>% 
 # drop_na(pop_percent_change20_21)
