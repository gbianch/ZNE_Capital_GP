...
avg_emp_yoy <- MSA_employment %>% 
  ...
  mutate(avg_emp_YoY = round(avg_emp_YoY, digits = 3)) %>% 
  drop_na() 

MSA_emp_income <- income_potential_MSAs %>% 
  right_join(avg_emp_yoy) %>% 
  filter(year == "2021") %>% 
...
datatable(metro_house_prices, extensions = 'Buttons', options = list(
    dom = 'Bfrtip',
...
msa_seperated <- metro_house_prices %>% 
