...
potential_MSAs <- msa_pop_2021_data %>% 
  ...
  select(cbsa, MSA, state, pop_percent_change20_21) 
...
MSA_house_income <- MSA_income_data %>% # rename row names
  inner_join(potential_MSAs) %>% 
  select(cbsa, MSA, state,median_income, income_moe_USD) 
#select(cbsa, MSA, state, pop_percent_change20_21, median_income, income_moe_USD) 

```

```{r, household income visual, fig.height = 7, fig.align='center'}
...
MSA_employment <- unemployment_data %>% 
  filter(year >= 2015) %>% 
  full_join(MSA_house_income) %>% 
  drop_na() %>% 
  select(cbsa, MSA, year, month, employment_rate, median_income, income_moe_USD)
...
MSA_median_rent2021_data <- read_csv(here("data", "raw", "2021MSA_medianrent.csv")) %>% 
  ...
  select(cbsa, rent50_0, rent50_1, rent50_2, rent50_3, rent50_4) %>% 
...
```{r, house price visual, fig.height = 7, fig.align='center'}
...
```{r property taxes data wrangling}
...
```{r property tax visual, fig.height = 7, fig.align='center'}
