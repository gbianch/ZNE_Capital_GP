...
climate_risk_data <- read_sheet("https://docs.google.com/spreadsheets/d/1HZCeNGf61RJ9GJs3cLJcl9aNpvmPMBF1Yoss2fytltc/edit#gid=2045475301") %>% 
...
criteria_weights_students <- c(0.21, 0.0, 0.12, 0.17, 0.16, 0.17, 0.17)

#make landlord_score column
criteria_all <- criteria_all %>% 
  mutate(total_score_students = cbsa, #this is just a place holder
         real_estate_score = as.numeric(real_estate_score),
         landlord_score = as.numeric(landlord_score)
  )
...
```{r climate risk weighted 50 and other 0.1}
# weight criteria for Owen
weights_climate50 <- c(0.1, 0.1, 0.1, 0.1, 0.0, 0.1, 0.50)

#make landlord_score column
criteria_all <- criteria_all %>% 
  mutate(score_climate50 = cbsa, #this is just a place holder
         real_estate_score = as.numeric(real_estate_score),
         landlord_score = as.numeric(landlord_score)
  )
...
for (i in 1:size[1]) {
  ...
  criteria_all$score_climate50[i] = weighted.mean(cbsa_data, weights_climate50)
}
...
```{r climate risk 40 }
# weight criteria for Owen
weights_climate40 <- c(0.15, 0.1, 0.15, 0.1, 0.0, 0.1, 0.40)

#make landlord_score column
criteria_all <- criteria_all %>% 
  mutate(score_climate40 = cbsa, #this is just a place holder
         real_estate_score = as.numeric(real_estate_score),
         landlord_score = as.numeric(landlord_score)
  )
...
for (i in 1:size[1]) {
  ...
  criteria_all$score_climate40[i] = weighted.mean(cbsa_data, weights_climate40)
}
```


```{r climate risk 50 and proportional weights for others}
...
```{r climate risk 35 and Jan 31 weights from owen}
...
sheet_write(criteria_all, ss = "https://docs.google.com/spreadsheets/d/1yqjhJvXUcEiC3qiYWkKNlF6NNpYXWg15zZHwmwpS5Q0/edit#gid=0")
```

```{r}
column_order <- c("real_estate_weighted", "rental_weighted", "financial_weighted", "electricity_weighted", "climate_impact_weighted", "climate_risk_weighted", "health_weighted", "cbsa", "city_msa", "state")

weighted_criteria <- criteria_all %>% 
  rename(real_estate_weighted = real_estate_score,
        rental_weighted = landlord_score,
        financial_weighted = normalized_irr,
        electricity_weighted = normalized_annual_renewable,
        climate_impact_weighted = inverted_n_total_co2,
        climate_risk_weighted = normalized_inverted_climate_risk,
        health_weighted = inverted_n_health_emissions_cost)
  
weighted_criteria <- weighted_criteria[, column_order]
