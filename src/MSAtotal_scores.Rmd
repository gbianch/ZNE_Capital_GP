...
```{r student weights with EAL, eval = FALSE}
size <- dim(criteria_all)
for (i in 1:size[1]) {
  cbsa_data <- c(criteria_all$real_estate_score[i],
                criteria_all$landlord_score[i],
...
#make landlord_score column
criteria_all <- criteria_all %>% 
  mutate(owen_FINAL_score = cbsa, #this is just a place holder
...
for (i in 1:size[1]) {
  ...
  criteria_all$owen_FINAL_score[i] = weighted.mean(cbsa_data, weights_climate50_prop) 
}

```

```{r Climate with 50 & proportional others with EAL, eval=FALSE}
...
weighted_criteria <- read_sheet(ss = "https://docs.google.com/spreadsheets/d/1yqjhJvXUcEiC3qiYWkKNlF6NNpYXWg15zZHwmwpS5Q0/edit#gid=0", sheet = "wtd_criteria_Owen")


score_tidy <- weighted_criteria %>% 
  pivot_longer(cols = c(4:9), # selecting criteria cols
               names_to = "criteria",
               values_to = "criteria_score") %>% 
 filter(city_msa != "Portland") # removing portland (negative number)



ggplot(data = score_tidy, aes(y = reorder(cbsa, FINAL_owen_SCORE), x = criteria_score)) + 
    geom_col(aes(fill = criteria)) +
  labs(x = "Total Score (Client)",
       y = " ") +
  guides(fill=guide_legend(title="Criteria (weighted)")) +
   scale_fill_manual(values = c("#FDB462", "gold", "palevioletred1",  "#BC80BD", "#6699CC", "#44AA99"),
                     labels=c('Climate Risk', 'CO2 Emissions', 'Electricity', "Finances", "Real Estate", "Landlord")) +
  scale_y_discrete((labels = score_tidy$city_msa))
