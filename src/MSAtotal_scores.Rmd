...
ggplot(data = score_tidy, aes(y = reorder(cbsa, FINAL_owen_SCORE), x = criteria_score, fill = criteria_labels)) + 
    geom_col(aes(fill = criteria)) +
  labs(x = "Total Score (Client)",
       y = " ") +
  guides(fill=guide_legend(title="Criteria (weighted)")) +
scale_fill_discrete(labels=c('Climate Risk', 'CO2 Emissions', 'Electricity', "Finances", "Real Estate", "Landlord")) +
