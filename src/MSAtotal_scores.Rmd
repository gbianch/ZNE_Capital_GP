---
title: "Total Score"
author: "Grace Bianchi"
date: '2022-11-14'
output: 
  html_document: 
    fig_caption: yes
editor_options: 
  chunk_output_type: console
---

```{r setup, include=FALSE, warning=FALSE, message=FALSE}
knitr::opts_chunk$set(echo = TRUE, warning = FALSE, message = FALSE)

library(tidyverse)
library(janitor)
library(googlesheets4)
library(dplyr)
```

```{r}
# read in the data
real_estate_score_data <- read_sheet("https://docs.google.com/spreadsheets/d/1Ssmb7pUd4NFZocVp9C75SRMOV6tAJCCt0TjrCYBB8pY/edit#gid=1038418521") %>% 
  select(cbsa, real_estate_score)

rental_policy_score_data <- read_sheet("https://docs.google.com/spreadsheets/d/1nLpDhM88a3rEBDVK5RZlk2B6QuwVIwvsfE5HeBz0mLo/edit#gid=983090810") %>% 
  select(cbsa, landlord_score)

reopt_data <- read_sheet("https://docs.google.com/spreadsheets/d/1tPqpmS8JOeRyunEmSyFYPYwshKDGx54Z-MISgYO3wZE/edit#gid=1874679054") %>% 
  select(cbsa, city_msa, state, normalized_irr, inverted_n_total_co2, inverted_n_health_emissions_cost, normalized_annual_renewable)

climate_risk_data <- read_sheet("https://docs.google.com/spreadsheets/d/1HZCeNGf61RJ9GJs3cLJcl9aNpvmPMBF1Yoss2fytltc/edit#gid=2045475301", sheet = "climate_risk+EAL") %>% 
  select(cbsa, normalized_inverted_climate_risk)

```


```{r read in data}
criteria_all <- real_estate_score_data %>% 
  inner_join(rental_policy_score_data)  %>% 
  inner_join(reopt_data) %>% 
  inner_join(climate_risk_data) # combine all criteria scores into one dataframe

# reorder columns
col_order <-c("cbsa", "city_msa", "state", "real_estate_score", "landlord_score", "normalized_annual_renewable", "normalized_irr", "inverted_n_total_co2",  "inverted_n_health_emissions_cost", "normalized_inverted_climate_risk")

criteria_all <- criteria_all[, col_order] %>% 
  rename(financial_score = normalized_irr,
         co2_score = inverted_n_total_co2,
         health_impact_score = inverted_n_health_emissions_cost,
         electricity_score = normalized_annual_renewable,
         climate_risk_score = normalized_inverted_climate_risk) %>% 
  mutate(real_estate_score = as.numeric(real_estate_score),
         landlord_score = as.numeric(landlord_score)) 

```



```{r FINAL student weights}
# weight criteria for students
criteria_weights_students <- c(0.18, 0.0, 0.10, 0.19, 0.17, 0.14, 0.22)

#make landlord_score column
criteria_all <- criteria_all %>% 
  mutate(equity_scenario_score = cbsa,
         client_FINAL_score = cbsa) #this is just a place holder
  
for (i in 1:nrow(criteria_all)) {
  cbsa_data <- c(criteria_all$real_estate_score[i],
                criteria_all$landlord_score[i],
                 criteria_all$financial_score[i],
                 criteria_all$co2_score[i],
                criteria_all$health_impact_score[i],
                criteria_all$electricity_score[i],
                criteria_all$climate_risk_score[i]
                  )
  criteria_all$equity_scenario_score[i] = weighted.mean(cbsa_data, criteria_weights_students)
}

```



```{r Client FINAL WEIGHTS}

client_weights <- c(0.18, 0.07, 0.11, 0.07, 0.0, 0.07, 0.50)

# multiply column by respective weight and sum for total
  
for (i in 1:nrow(criteria_all)) {
  cbsa_data <- c(criteria_all$real_estate_score[i],
                criteria_all$landlord_score[i],
                 criteria_all$financial_score[i],
                 criteria_all$co2_score[i],
                criteria_all$health_impact_score[i],
                criteria_all$electricity_score[i],
                criteria_all$climate_risk_score[i]
                  )
  criteria_all$client_FINAL_score[i] = weighted.mean(cbsa_data, client_weights) 
}

```


```{r}

criteria_all <- criteria_all %>% 
  mutate_if(is.numeric,
            round,
            digits = 5)

weighted <- criteria_all %>% 
  t(t(criteria_all[4:10])*criteria_weights_students) %>% 
  as.data.frame(row.names=criteria_all$cbsa) 

 calculate_wt_criteria <- function(data, weights){
    wt_criteria <- data %>%
      mutate(wt_real_estate = real_estate_score * weights[1],
             wt_landlord_policy = landlord_score * weights[2], 
             wt_solar_irr = financial_score * weights[3],
            wt_co2 = co2_score * weights[4], 
             wt_health_impacts = health_impact_score * weights[5],
             wt_electricity = electricity_score * weights[6],
             wt_climate_risk = climate_risk_score * weights[7]) %>% 
      select(!c(ends_with("_score"))) %>%
      rename("Real Estate"="wt_real_estate",
             "Landlord Policy"="wt_landlord_policy",
             "Electricity"="wt_electricity", 
             "CO2 Emissions"="wt_co2",
             "Climate Risk"="wt_climate_risk",
             "Health Impacts"="wt_health_impacts", 
             "Solar IRR"="wt_solar_irr") 
 }

 wt_client <- calculate_wt_criteria(criteria_all, client_weights)  %>%  
  unite("msa_state", city_msa:state, sep = ", ")
   
 
#sheet_write(criteria_all, ss = "https://docs.google.com/spreadsheets/d/1yqjhJvXUcEiC3qiYWkKNlF6NNpYXWg15zZHwmwpS5Q0/edit#gid=0", sheet = "FINAL_total_score")
```

### Data Visualization- barcharts

```{r client data viz}


# read in cbsa data with full names
client_score_tidy <- wt_client %>% 
  pivot_longer(cols = c(3:9), # selecting criteria cols
               names_to = "criteria",
               values_to = "wt_criteria_score") %>% 
 filter(msa_state != "Portland, ME")  # removing portland (negative number)


tiff(filename = "wt_criteria_client_chart.tiff",width = 8, height = 6, units = "in", res = 300, pointsize = 12)
p1 <- ggplot(data = client_score_tidy, aes(y = reorder(msa_state,  wt_criteria_score, FUN=sum), x = wt_criteria_score)) +
    geom_col(aes(fill = criteria)) +
  labs(x = "Total Score",
       y = " ",
       title = "") +
  theme_bw() +
  guides(fill=guide_legend(title="Criteria (weighted)")) +
   scale_fill_manual(values = c("palevioletred1","orange",  "gold", "#44AA99", "dodgerblue4", "#BC80BD", "#6495ED"),
      breaks = c("Climate Risk", "CO2 Emissions", "Electricity", "Solar IRR", "Real Estate", "Landlord Policy", "Health Impacts")) +
        scale_x_continuous(limits = c(0, 1.0), expand = c(0,0), breaks=seq(0, 01.0, 0.1)) +
  theme(legend.position = "none")
dev.off()
```

```{r student data viz}
## read in weighted criteria sheet
student_weighted_criteria <- read_sheet(ss = "https://docs.google.com/spreadsheets/d/1yqjhJvXUcEiC3qiYWkKNlF6NNpYXWg15zZHwmwpS5Q0/edit#gid=0", sheet = "wtd_criteria_stud") 

equity_wt <- calculate_wt_criteria(criteria_all, criteria_weights_students) %>% 
  unite("msa_state", city_msa:state, sep = ", ")


# read in cbsa data with full names
score_tidy2 <- equity_wt %>% 
  pivot_longer(cols = c(3:9), # selecting criteria cols
               names_to = "criteria",
               values_to = "wt_criteria_score") 

tiff(filename = "wt_criteria_student_chart.tiff", width = 8, height = 6, units = "in", res = 300)

p2 <- ggplot(data = score_tidy2, aes(y = reorder(msa_state, wt_criteria_score, FUN=sum), x = wt_criteria_score)) + 
    geom_col(aes(fill = criteria)) +
  labs(x = "Total Score",
       y = " ",
       title = "") +
  theme_bw() +
  guides(fill=guide_legend(title="Criteria (weighted)")) +
   scale_fill_manual(values = c("palevioletred1", "orange",  "gold", "#44AA99", "dodgerblue4", "#6495ED", "#BC80BD"),
                     labels=c('Climate Risk', 'CO2 Emissions', 'Electricity', "Solar IRR", "Real Estate", "Health Impacts", "Landlord Policy")) +
  scale_x_continuous(limits = c(0, 1.0), expand = c(0,0), breaks=seq(0, 1.0, 0.1)) 

dev.off()
```

```{r}
library(patchwork)


p1 <- ggplot(data = client_score_tidy, aes(y = reorder(msa_state,  wt_criteria_score, FUN=sum), x = wt_criteria_score)) +
    geom_col(aes(fill = criteria)) +
  labs(x = "Total Score",
       y = " ",
       title = "") +
  theme_bw() +
  guides(fill=guide_legend(title="Criteria (weighted)")) +
   scale_fill_manual(values = c("palevioletred1","orange",  "gold", "#44AA99", "dodgerblue4", "#BC80BD", "#6495ED"),
      breaks = c("Climate Risk", "CO2 Emissions", "Electricity", "Solar IRR", "Real Estate", "Landlord Policy", "Health Impacts")) +
        scale_x_continuous(limits = c(0, 1.0), expand = c(0,0), breaks=seq(0, 01.0, 0.1)) +
  theme(legend.position = "none")

p2 <- ggplot(data = score_tidy2, aes(y = reorder(msa_state, wt_criteria_score, FUN=sum), x = wt_criteria_score)) + 
    geom_col(aes(fill = criteria)) +
  labs(x = "Total Score",
       y = " ",
       title = "") +
  theme_bw() +
  guides(fill=guide_legend(title="Criteria (weighted)")) +
   scale_fill_manual(values = c("palevioletred1", "orange",  "gold", "#44AA99", "dodgerblue4", "#6495ED", "#BC80BD"),
                     labels=c('Climate Risk', 'CO2 Emissions', 'Electricity', "Solar IRR", "Real Estate", "Health Impacts", "Landlord Policy")) +
  scale_x_continuous(limits = c(0, 1.0), expand = c(0,0), breaks=seq(0, 1.0, 0.1)) 

tiff(filename = "data_vz.tiff", width=12, units = "in", res=100)
p3<- p1+p2 + plot_layout(widths = c(6, 6)) +
  plot_annotation(tag_levels = 'A',
                  caption = "Figure 1. Ranked metropolitan areas (n=29) based on investment favorability for rooftop solar PV on mutlifamily houses. Areas were scored using six criteria, each given a respective weight given stakeholder priorities. \n Each criteria is comprised of indicator metrics using data from the U.S.  Census Bureau and NREL's reOpt tool. Graph A represents a real estate investor stakeholder who prioritizes climate risk due to long-term investments. \n B represents a equity-focused stakeholder who prioritizes markets that reduce CO2 emissions and associated health impacts.")
dev.off()



```

"Figure 1. Ranked metropolitan areas (n=29) based on investment favorability for rooftop solar PV on mutlifamily houses. Areas were scored using six criteria, each given a respective weight given stakeholder priorities. Each criteria is comprised of indicator metrics using data from the U.S. Census Bureau and NREL's reOpt tool. Graph A represents a real estate investor stakeholder who prioritizes climate risk due to long-term investments. B represents a equity-focused stakeholder who values impact one 